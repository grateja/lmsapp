        ...
        fun update() {
            ...
            if(tab == TAB_MACHINE_STUB) {
                ...
                if((itemizedJo == true && tab == TAB_JOB_ORDER) || (itemizedClaimStub == true && tab == TAB_CLAIM_STUB)) {
                    items.addAll(
                        jobOrder?.services?.filter { it.deletedAt == null }?.takeIf { it.isNotEmpty() }?.map {
                    ...
                    items.addAll(
                        jobOrder?.products?.filter { it.deletedAt == null }?.takeIf { it.isNotEmpty() } ?.map {
                    ...
                    items.addAll(
                        jobOrder?.extras?.filter { it.deletedAt == null }?.takeIf { it.isNotEmpty() }?.map {
                    ...
                    items.addAll(
                        jobOrder?.deliveryCharge?.takeIf { it.deletedAt == null }?.let {
                            listOf(
                ...
                val discountInPeso = jobOrder?.takeIf { it.discount?.deletedAt == null }?.discountInPeso() ?: 0.0f

                val discount = jobOrder?.takeIf { it.discount?.deletedAt == null }?.discount?.let { discount ->
