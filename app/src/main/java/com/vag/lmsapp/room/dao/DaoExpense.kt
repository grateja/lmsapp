...
abstract class DaoExpense : BaseDao<EntityExpense> {
    ...
    @Query("SELECT * FROM expenses WHERE remarks LIKE '%' || :keyword || '%' AND deleted = 0 " +
    ...
    @Query("SELECT tag, COUNT(*) as count, SUM(amount) as sum FROM expenses WHERE deleted = 0 AND date(created_at / 1000, 'unixepoch', 'localtime') = :dateFrom OR ( :dateTo IS NOT NULL AND date(created_at / 1000, 'unixepoch', 'localtime') BETWEEN :dateFrom AND :dateTo ) GROUP BY tag")
