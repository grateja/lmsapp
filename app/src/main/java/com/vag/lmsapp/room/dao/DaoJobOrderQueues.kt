...
interface DaoJobOrderQueues {
    @Query("""
        SELECT SUM(quantity - used) as available, customer_id, address, crn, jos.svc_machine_type as machine_type, cu.name as customer_name, MAX(jo.created_at) AS latest_job_order
        FROM job_orders jo
